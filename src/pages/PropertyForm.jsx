...
  // ─── OTOMATİK TARAMA STATE ───────────────────────────────────────────────────
  const [autoScanActive, setAutoScanActive] = useState(false);
  const [autoScanSteps, setAutoScanSteps] = useState({}); // key -> 'loading' | 'done' | 'error'
...
        
        // ⚡ Konum çözümlenir çözümlenmez anında otomatik Ref No üretimi (GECİKMESİZ)
        let autoRef = '';
        if (isNew) {
...
    // 2. Havalimanları (Dinamik Overpass Arama İstasyonu)
...
  // ─── ⚡ EL İLE DEĞİŞİKLİK YAPILIRSA DA ÇALIŞAN GARANTÖR YAPI ────────────────
  useEffect(() => {
    if (!isNew) return; 
...
              <Radar className="w-4 h-4 text-rose-600 animate-pulse" /> Ulaşım & Sosyal Çevre Radarı
            </h3>
...
              {radarLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : '🌐'} Otomatik Tara
            </button>
          </div>
